...
class DiagnosticApp(Sub2LRCApp):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(panel, text="开始记录", command=self._start_latency_recording).pack(side="left")
        ttk.Button(panel, text="停止记录", command=self._stop_latency_recording).pack(
            side="left", padx=(6, 10)
        )
        ttk.Label(panel, textvariable=self.latency_result, anchor="w").pack(
