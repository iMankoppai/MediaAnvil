...
“音频预览”页支持 MP3、WAV、FLAC、M4A、AAC 和 OGG，提供播放、暂停、进度跳转、当前/总时长及音量控制。播放进度和音量使用圆形滑块，点击轨道会直接移动到点击位置，也可以连续拖动；拖动播放进度时歌词和时间会同步预览，松开后音频跳转。空格键会在播放和暂停之间切换。它只读取并播放当前选择的单个文件，不会修改音频或建立歌单；切换文件或关闭软件时会自动停止并释放旧的播放进程。
