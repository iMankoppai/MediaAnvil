...
音频编码由 FFmpeg 完成。请安装 FFmpeg 并加入系统 `PATH`，或者把 `ffmpeg.exe` 放在 `Sub2LRC.exe` 的同一目录。找不到 FFmpeg 时程序会显示明确提示。输出默认保留原文件名并将 `.wav` 改为 `.mp3`；如果同名 MP3 已存在，会生成 `_1`、`_2` 等安全的新文件名。
...
- 当前版本仅提供 Windows EXE。
- 音频转换依赖 FFmpeg；FFmpeg 不包含在 Sub2LRC 单文件 EXE 中。
