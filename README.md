...
如果音频旁存在同名 `.lrc`，程序会优先载入该歌词；MP3 没有同名歌词时会尝试读取内嵌的 LRC 时间歌词。播放时居中显示纯歌词列表，当前句使用深黄色文字提示，不显示背景框，并按实际显示位置滚动到视图中部；鼠标移到歌词上会显示浅黄色行背景，点击即可跳转并让该句保持在视图中部。没有同步歌词时仍可正常播放。
