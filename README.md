...
音频未播放或暂停时不启动任何播放进度轮询，也不监听窗口移动事件；只有开始播放后才定时同步进度和歌词，暂停后立即停止轮询，避免空闲定时器干扰窗口拖动。

除“音频预览”外，其他功能页面按需创建；切换页面会释放上一功能页的界面控件，并在返回时恢复文件列表、转换设置和尚未保存的标签修改，避免大量隐藏控件长期拖慢窗口移动。

“音频标签编辑”在尚未选择音频时只显示文件选择区；成功读取音频后才创建基础信息、歌词、封面和保存区域，避免空编辑页建立无用控件。

文件打开、批量选择、目录选择和另存为现统一使用应用内选择器，不再调用 `tk_getOpenFile`、`tk_chooseDirectory` 等原生 Tk 对话框。选择器支持中文路径、扩展名过滤、多选和已有文件二次确认覆盖，并将文件选择造成的窗口状态变化隔离在可销毁的子窗口内。

Windows 桌面路径从系统注册的已知文件夹读取，支持 OneDrive 重定向桌面，不再假定桌面固定为用户目录下的 `Desktop`。
