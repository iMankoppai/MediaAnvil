...
如果音频旁存在同名 `.lrc`，程序会优先载入该歌词；MP3 没有同名歌词时会尝试读取内嵌的 LRC 时间歌词。播放时居中显示纯歌词列表，当前句使用深黄色文字提示并固定在接近垂直中心的锚点，其他歌词随进度从下向上滚动；首尾留白保证第一句和最后一句也能到达相同位置。鼠标移到歌词上会显示浅黄色行背景，点击即可跳转并让该句回到固定锚点。没有同步歌词时仍可正常播放。
