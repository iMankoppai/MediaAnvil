...
EN = {
    ...
    '音频合并 / 分割': 'Join / Split',
    '把多段音频接成一条，或把一条音频切成多段；输出为新文件，不改动源文件。': 'Join several audio files into one, or cut one into pieces. Output is a new file; sources are never changed.',
    '批量处理 · 让音频拼接与切分更省事  —': 'Batch processing · Simpler audio joining and splitting  —',
    '合并为一个文件': 'Join into one file', '分割为多个文件': 'Split into several files',
    '处理方式': 'Mode', '选择合并多段音频，或把一段音频切成多段': 'Join several files, or cut one file into pieces',
    '分段方式': 'Split by', '按段数等分': 'Equal parts', '按每段时长': 'Fixed length',
    '段数': 'Parts', '每段': 'Each', '开始处理': 'Start', '开始合并': 'Start Joining', '开始分割': 'Start Splitting',
    '处理结果': 'Results', '显示本次生成的每一个文件': 'Every file produced by this run',
    '尚未开始': 'Not started yet', '选择文件后显示时长': 'Select files to see the plan',
    '分割只处理列表中的第一个文件': 'Splitting uses the first file in the list',
    '合并至少需要两个音频文件。': 'Joining needs at least two audio files.',
    '请先添加并勾选要处理的文件。': 'Add and select at least one file first.',
    '输出设置': 'Output', '输出为新文件，源文件保持不变': 'Written to new files; sources are kept',
    '完成：共生成': 'Done: ',
    '支持 MP3、WAV、FLAC、M4A、AAC、OGG 等格式': 'Supports MP3, WAV, FLAC, M4A, AAC, OGG and more',
    '淡入淡出': 'Fade in / out', '音量标准化': 'Normalise volume',
    '音频开头淡入、结尾淡出的时长': 'Fade the start in and the end out over this many seconds',
    '按 EBU R128 把各段响度调整到一致，适合拼接音量不同的录音': 'Level every part to the same loudness (EBU R128); useful when joining recordings at different volumes',
    '两项默认关闭；勾选后仅作用于新生成的文件。': 'Both are off by default and only affect newly created files.',
    '合并需要至少两个文件；分割只需一个': 'Joining needs at least two files; splitting needs one',
    '上移': 'Move Up', '下移': 'Move Down',
    '把选中的文件向上移动一位': 'Move the selected files up one place',
    '把选中的文件向下移动一位': 'Move the selected files down one place',
    '请先在列表中选择要调整顺序的文件。': 'Select the files you want to reorder first.',
    '拖动条目可调整顺序；也可从资源管理器拖入文件或文件夹': 'Drag an entry to reorder; you can also drop files or folders from Explorer',
    ...
    '曲目号': 'Track Number', '年份': 'Year', '流派': 'Genre',
    '请输入歌名': 'Enter title', '请输入歌手': 'Enter artist', '请输入专辑': 'Enter album',
    '如 3 或 3/12': 'e.g. 3 or 3/12', '如 2024': 'e.g. 2024', '如 流行 / Rock': 'e.g. Pop / Rock',
    ...
    '提前': 'Earlier', '延后': 'Later', '应用': 'Apply', '重置': 'Reset',
    '要调整的秒数': 'Seconds to adjust',
    '请先设置偏移秒数。': 'Set an offset in seconds first.',
    '请先导入或载入歌词。': 'Import or load lyrics first.',
    '歌词中没有可调整的时间戳。': 'The lyrics contain no timestamps to adjust.',
    ...
    '批量写入标签': 'Batch Write Tags', '批量标签结果': 'Batch Tag Results',
    '批量偏移歌词': 'Batch Shift Lyrics', '批量歌词偏移结果': 'Batch Lyric Shift Results',
    '歌词偏移': 'Lyric Offset',
    '批量写入同一封面': 'Batch Write Same Artwork', '批量封面结果': 'Batch Artwork Results',
    '选择封面图片…': 'Choose Artwork…', '选择封面图片': 'Choose Artwork',
    '未选择封面图片': 'No artwork chosen', '请先选择封面图片。': 'Choose an artwork image first.',
    '请填写要调整的秒数。': 'Enter the number of seconds to shift by.',
    '跳过：偏移秒数为 0，没有需要调整的歌词。': 'Skipped: the offset is 0 seconds, so no lyrics needed changing.',
    '请先扫描音乐文件夹。': 'Scan a music folder first.',
    '请至少填写一个要批量写入的标签。': 'Fill in at least one tag to write in bulk.',
    '跳过：没有填写任何要批量写入的字段。': 'Skipped: no field was filled in for the batch write.',
