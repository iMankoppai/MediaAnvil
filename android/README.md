...
- **音频格式转换**：批量转换为 M4A（AAC）、FLAC、OGG，使用系统编码器（Media3 Transformer），结果保存在源文件旁边；MP3 输出受系统限制暂不支持。
- **图片格式转换**：JPG、PNG、WebP、BMP 互转，可调质量（JPG/WebP）；透明区域转 JPG/BMP 时填白。
- **批量重命名**：按 `{artist}`、`{title}`、`{album}`、`{track}`、`{year}` 模板批量重命名文件夹内音频，预览冲突与自动避让，支持撤销最近一次批量。
- 标签编辑入口与每首歌的"媒体工具"抽屉保持不变。

## 尚未实现

与桌面版相比仍缺少：封面裁剪的数值输入框、音频转换导出 MP3、设置页的部分高级选项。这些会随后续版本逐步补齐。
